...
describe("lerna-publish-bun", () => {
  let fixture: Fixture;

  beforeEach(async () => {
    fixture = await Fixture.create({
      e2eRoot: process.env.E2E_ROOT,
      name: "lerna-publish-bun",
      packageManager: "bun",
      initializeGit: true,
  ...
  describe("from-package", () => {
    it("should publish packages that are not in registry with bun", async () => {
      ...
      await fixture.exec(`git commit -m "chore: bump version to ${version}"`);
      await fixture.exec("git push origin test-main");

      const output = await fixture.lerna("publish from-package -y");
      const unpublishOutput = await fixture.exec(
      ...
      expect(replaceVersion(output.combinedOutput)).toContain("test-2 => XX.XX.XX");
      expect(replaceVersion(output.combinedOutput)).toContain("lerna success published 1 package");
      expect(replaceVersion(unpublishOutput.combinedOutput)).toContain("test-2@XX.XX.XX");
    });
  });
});
