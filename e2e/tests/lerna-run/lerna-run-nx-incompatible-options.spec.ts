...
describe("lerna-run-nx-incompatible-options", () => {
  ...
  it("--parallel should warn", async () => {
    ...
    expect(output.combinedOutput).toMatchInlineSnapshot(`
      ...
      lerna notice cli v999.9.9-e2e.0
      lerna WARN run "parallel", "sort", and "no-sort" are ignored when nx.json has targetDefaults defined. See https://lerna.js.org/docs/recipes/lerna6-obsolete-options for details.
