...
import BootstrapProxyCommand from "../src/commands/BootstrapProxyCommand";
...
describe("BootstrapProxyCommand", () => {

  describe("dependencies between packages in the repo", () => {
    let testDir;

    beforeEach((done) => {
      testDir = initFixture("BootstrapProxyCommand/basic", done);
    });

    it("should bootstrap files", (done) => {
      const bootstrapProxyCommand = new BootstrapProxyCommand([], {});

      bootstrapProxyCommand.runValidations();
      bootstrapProxyCommand.runPreparations();
      ...
      bootstrapProxyCommand.runCommand(exitWithCode(0, (err) => {
  ...
  describe("external dependencies that haven't been installed", () => {
    ...
    beforeEach((done) => {
      testDir = initFixture("BootstrapProxyCommand/cold", done);
    });
  ...
  describe("external dependencies that have already been installed", () => {
    ...
    beforeEach((done) => {
      testDir = initFixture("BootstrapProxyCommand/warm", done);
    });
