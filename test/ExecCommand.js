  ...
  describe("in a basic repo", () => {
    ...
    it("should run a command", (done) => {
      ...
      stub(ChildProcessUtilities, "spawn", (command, args, options, callback) => {
        ...
        if (calls === 0) assert.deepEqual(options, { cwd: path.join(testDir, "packages/package-1"), env: Object.assign(process.env, { PACKAGE_NAME: "package-1" }) });
        if (calls === 1) assert.deepEqual(options, { cwd: path.join(testDir, "packages/package-2"), env: Object.assign(process.env, { PACKAGE_NAME: "package-2" }) });
    ...
    it("should run a command with parameters", (done) => {
      ...
      stub(ChildProcessUtilities, "spawn", (command, args, options, callback) => {
        ...
        if (calls === 0) {
          assert.deepEqual(options, { cwd: path.join(testDir, "packages/package-1"), env: Object.assign(process.env, { PACKAGE_NAME: "package-1" }) });
          assert.deepEqual(args, ["-la"]);
        }
        if (calls === 1) {
          assert.deepEqual(options, { cwd: path.join(testDir, "packages/package-2"), env: Object.assign(process.env, { PACKAGE_NAME: "package-2" }) });
          assert.deepEqual(args, ["-la"]);
        }
