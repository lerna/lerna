...
- First, publish each module in `packages` that has been updated since the last version to npm with the [dist-tag](https://docs.npmjs.com/cli/dist-tag) `lerna-temp`.
...
- Second, after all packages have been published, remove the `lerna-temp` versions and add the versions to `latest`.
