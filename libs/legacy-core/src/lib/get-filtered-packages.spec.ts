...
describe("getFilteredPackages", () => {
  ...
  `("filters $argv", async ({ argv, matched }) => {
    ...
    const execOpts = { cwd };
    const options = parseOptions(...argv) as Partial<FilterOptions>;
  ...
  test("--since returns all packages if no tag is found", async () => {
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--since") as Partial<FilterOptions>;
  ...
  test("--since <ref> should return packages updated since <ref>", async () => {
    ...
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--since", "deadbeef") as Partial<FilterOptions>;
  ...
  test("--scope package-{2,3,4} --since main", async () => {
    ...
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--scope", "package-{2,3,4}", "--since", "main") as Partial<FilterOptions>;
  ...
  test("--exclude-dependents", async () => {
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--since", "foo", "--exclude-dependents") as Partial<FilterOptions>;
  ...
  test("--include-dependents", async () => {
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--scope", "package-1", "--include-dependents") as Partial<FilterOptions>;
  ...
  test("--include-dependencies", async () => {
    const packageGraph = await buildGraph(cwd);
    const execOpts = { cwd };
    const options = parseOptions("--scope", "package-3", "--include-dependencies") as Partial<FilterOptions>;
