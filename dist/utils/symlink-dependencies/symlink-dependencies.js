...
const {
  createSymlink
} = require("@lerna/create-symlink");

const {
  resolveSymlink
} = require("@lerna/resolve-symlink");
...
function symlinkDependencies(packages, packageGraph, tracker) {
  ...
  const nodes = packageGraph.size === packages.length ? packageGraph.values() : new Set(packages.map(({
    name
  ...
  return pMapSeries(nodes, currentNode => {
    ...
    return pMap(currentNode.localDependencies, ([dependencyName, resolved]) => {
      ...
      let chain = Promise.resolve(); // check if dependency is already installed
      ...
      chain = chain.then(dirExists => {
        if (dirExists) {
          ...
          if (isDepSymlink !== false && isDepSymlink !== dependencyNode.location) {
            ...
            tracker.warn("EREPLACE_OTHER", `Symlink already exists for ${dependencyName} dependency of ${currentName}, ` + "but links to different location. Replacing with updated symlink...");
          } else if (isDepSymlink === false) {
            // installed dependency is not a symlink
            tracker.warn("EREPLACE_EXIST", `${dependencyName} is already installed for ${currentName}. Replacing with symlink...`); // remove installed dependency
      ...
      const dependencyLocation = dependencyNode.pkg.contents ? path.resolve(dependencyNode.location, dependencyNode.pkg.contents) : dependencyNode.location;
      chain = chain.then(() => createSymlink(dependencyLocation, targetDirectory, "junction")); // TODO: pass PackageGraphNodes directly instead of Packages
