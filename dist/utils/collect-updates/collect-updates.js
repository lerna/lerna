...
const {
  hasTags
} = require("./lib/has-tags");

const {
  collectPackages
} = require("./lib/collect-packages");

const {
  getPackagesForOption
} = require("./lib/get-packages-for-option");

const {
  makeDiffPredicate
} = require("./lib/make-diff-predicate");
...
function collectUpdates(filteredPackages, packageGraph, execOpts, commandOptions) {
  const {
    ...
    excludeDependents
  } = commandOptions; // If --conventional-commits and --conventional-graduate are both set, ignore --force-publish
  ...
  const packages = filteredPackages.length === packageGraph.size ? packageGraph : new Map(filteredPackages.map(({
    name
  }) => [name, packageGraph.get(name)]));
  ...
  if (hasTags(execOpts)) {
    ...
    const {
      ...
      refCount,
      lastTagName
    } = describeRefSync(execOpts, commandOptions.includeMergedTags); // TODO: warn about dirty tree?
