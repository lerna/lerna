...
function packDirectory(_pkg, dir, options) {
  ...
  chain = chain.then(() => pkg.refresh());
  chain = chain.then(() => packlist({
    path: pkg.contents
  }));
  chain = chain.then(files => tar.create({
    cwd: pkg.contents,
    prefix: "package/",
    portable: true,
  ...
  chain = chain.then(tarFilePath => getPacked(pkg, tarFilePath).then(packed => Promise.resolve().then(() => runLifecycle(pkg, "postpack", opts)).then(() => packed)));
  return chain;
}

function getTarballName(pkg) {
  const name = pkg.name[0] === "@" ? // scoped packages get special treatment
  pkg.name.substr(1).replace(/\//g, "-") : pkg.name;
