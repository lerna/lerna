...
function npmPublish(pkg, tarFilePath, options = {}, otpCache) {
  ...
  const {
    scope
  } = npa(pkg.name); // pass only the package scope to libnpmpublish
  ...
  if (!dryRun) {
    ...
    chain = chain.then(([tarData, manifest]) => {
      ...
      return otplease(innerOpts => publish(manifest, tarData, innerOpts), opts, otpCache).catch(err => {
        opts.log.silly("", err);
        opts.log.error(err.code, err.body && err.body.error || err.message); // avoid dumping logs, this isn't a lerna problem

        err.name = "ValidationError"; // ensure process exits non-zero

        process.exitCode = "errno" in err ? err.errno : 1; // re-throw to break chain upstream
