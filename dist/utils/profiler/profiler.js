...
class Profiler {
  ...
  run(fn, name) {
    ...
    return Promise.resolve().then(() => {
      ...
      const duration = process.hrtime(startTime); // Trace Event Format documentation:
  ...
  output() {
    return fs.outputJson(this.outputPath, this.events).then(() => this.logger.info("profiler", `Performance profile saved to ${this.outputPath}`));
  }
