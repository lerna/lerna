...
function add(spec, tag, options, otpCache) {
  ...
  opts.log.verbose("dist-tag", `adding "${cleanTag}" to ${name}@${version}`); // istanbul ignore next
  ...
  return fetchTags(opts).then(tags => {
    ...
    return otplease(wrappedPayload => fetch(uri, wrappedPayload), payload, otpCache).then(() => {
      opts.log.verbose("dist-tag", `added "${cleanTag}" to ${name}@${version}`); // eslint-disable-next-line no-param-reassign
...
function remove(spec, tag, options, otpCache) {
  ...
  opts.log.verbose("dist-tag", `removing "${tag}" from ${opts.spec.name}`); // istanbul ignore next
  ...
  return fetchTags(opts).then(tags => {
    ...
    return otplease(wrappedPayload => fetch(uri, wrappedPayload), payload, otpCache).then(() => {
      opts.log.verbose("dist-tag", `removed "${tag}" from ${opts.spec.name}@${version}`); // eslint-disable-next-line no-param-reassign
...
function fetchTags(opts) {
  return fetch.json(`/-/package/${opts.spec.escapedName}/dist-tags`, { ...opts,
    preferOnline: true,
    spec: opts.spec
  }).then(data => {
