...
module.exports = mapToRegistry; // https://github.com/npm/npm/blob/f644018/lib/utils/map-to-registry.js

function mapToRegistry(name, config) {
  log.silly("mapToRegistry", "name", name);
  let registry; // the name itself takes precedence
  ...
  const credentials = config.getCredentialsByURI(registry); // normalize registry URL so resolution doesn't drop a piece of registry URL
