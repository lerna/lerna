...
function formatJSONGraph(resultList, viewOptions) {
  ...
  const graph = {};
  const getNeighbors = viewOptions.showAll ? pkg => Object.keys(Object.assign({}, pkg.devDependencies, pkg.peerDependencies, pkg.optionalDependencies, pkg.dependencies)).sort() : pkg => Object.keys(Object.assign({}, // no devDependencies
  // no peerDependencies
...
function makeParseable(pkg) {
  const result = [pkg.location, pkg.name]; // sometimes the version is inexplicably missing?
