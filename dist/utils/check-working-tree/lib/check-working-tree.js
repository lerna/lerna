...
const {
  collectUncommitted
} = require("@lerna/collect-uncommitted");
...
function checkWorkingTree({
  ...
  chain = chain.then(() => describeRef({
    cwd
  })); // wrap each test separately to allow all applicable errors to be reported

  const tests = [// prevent duplicate versioning
  chain.then(throwIfReleased), // prevent publish of uncommitted changes
  chain.then(mkThrowIfUncommitted({
    cwd
...
function mkThrowIfUncommitted(options = {}) {
  return function throwIfUncommitted({
    isDirty
  }) {
    if (isDirty) {
