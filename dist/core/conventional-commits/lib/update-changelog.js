...
const {
  getChangelogConfig
} = require("./get-changelog-config");

const {
  makeBumpOnlyFilter
} = require("./make-bump-only-filter");

const {
  readExistingChangelog
} = require("./read-existing-changelog");
...
function updateChangelog(pkg, type, {
  ...
  return getChangelogConfig(changelogPreset, rootPath).then(config => {
    ...
    if (type === "root") {
      context.version = version; // preserve tagPrefix because cc-core can't find the currentTag otherwise

      context.currentTag = `${tagPrefix}${version}`; // root changelogs are only enabled in fixed mode, and need the proper tag prefix
      ...
      if (type === "independent") {
        ...
        options.tagPrefix = tagPrefix; // preserve tagPrefix because cc-core can't find the currentTag otherwise
    ...
    return Promise.all([getStream(changelogStream).then(makeBumpOnlyFilter(pkg)), readExistingChangelog(pkg)]).then(([newEntry, [changelogFileLoc, changelogContents]]) => {
