...
const {
  logPackageError
} = require("./lib/log-package-error");

const {
  warnIfHanging
} = require("./lib/warn-if-hanging");
...
class Command {
  constructor(_argv) {
    ...
    log.silly("argv", argv); // "FooCommand" => "foo"

    this.name = this.constructor.name.replace(/Command$/, "").toLowerCase(); // composed commands are called from other commands, like publish -> version
    ...
    let runner = new Promise((resolve, reject) => {
      ...
      chain.then(result => {
        ...
        warnIfHanging(); // error code is handled by cli.fail()
    ...
    if (argv.onResolved || argv.onRejected) {
      runner = runner.then(argv.onResolved, argv.onRejected); // when nested, never resolve inner with outer callbacks
    ...
    for (const key of ["cwd", "$0"]) {
      Object.defineProperty(argv, key, {
        enumerable: false
      });
    }
  ...
  configureOptions() {
    ...
    const commandConfig = this.project.config.command || {}; // The current command always overrides otherCommandConfigs

    const overrides = [this.name, ...this.otherCommandConfigs].map(key => commandConfig[key]);
    this.options = defaultOptions( // CLI flags, which if defined overrule subsequent values
    this.argv, // Namespaced command options from `lerna.json`
    ...overrides, // Global options from `lerna.json`
    this.project.config, // Environmental defaults prepared in previous step
    this.envDefaults);
  }
