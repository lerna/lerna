...
} = require("./lib/cyclic-package-graph-node");

const {
  PackageGraphNode
} = require("./lib/package-graph-node");

const {
  reportCycles
} = require("./lib/report-cycles");
...
class PackageGraph extends Map {
  ...
  constructor(packages, graphType = "allDependencies", forceLocal) {
    ...
    this.forEach((currentNode, currentName) => {
      const graphDependencies = graphType === "dependencies" ? Object.assign({}, currentNode.pkg.optionalDependencies, currentNode.pkg.dependencies) : Object.assign({}, currentNode.pkg.devDependencies, currentNode.pkg.optionalDependencies, currentNode.pkg.dependencies);
      Object.keys(graphDependencies).forEach(depName => {
        const depNode = this.get(depName); // Yarn decided to ignore https://github.com/npm/npm/pull/15900 and implemented "link:"
  ...
  extendList(packageList, nodeProp) {
    ...
    const search = new Set(packageList.map(({
      name
    }) => this.get(name))); // an intermediate list of matched PackageGraphNodes
  ...
  remove(candidateNode) {
    ...
    this.forEach(node => {
      ...
      node.localDependencies.delete(candidateNode.name); // remove outgoing edges ("outdegree")
