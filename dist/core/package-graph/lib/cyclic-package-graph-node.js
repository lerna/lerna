...
class CyclicPackageGraphNode extends Map {
  ...
  toString() {
    const parts = Array.from(this, ([key, node]) => node.isCycle ? `(nested cycle: ${node.toString()})` : key); // start from the origin
  ...
  unlink(candidateNode) {
    ...
    this.localDependencies.delete(candidateNode.name); // remove outgoing edges ("outdegree")
