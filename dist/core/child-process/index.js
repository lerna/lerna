...
const logTransformer = require("strong-log-transformer"); // bookkeeping for spawned processes

/** @type {Set<import("execa").ExecaChildProcess<string>>} */


const children = new Set(); // when streaming processes are spawned, use this color for prefix

const colorWheel = ["cyan", "magenta", "blue", "yellow", "green", "red"];
const NUM_COLORS = colorWheel.length; // ever-increasing index ensures colors are always sequential
...
function spawnProcess(command, args, opts) {
  ...
  const drain = (exitCode, signal) => {
    children.delete(child); // don't run repeatedly if this is the error event
...
function wrapError(spawned) {
  if (spawned.pkg) {
    return spawned.catch(err => {
      ...
      err.exitCode = getExitCode(err); // log non-lerna error cleanly
