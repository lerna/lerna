...
function makeFileFinder(rootPath, packageConfigs) {
  ...
  return (fileName, fileMapper, customGlobOpts) => {
    ...
    const promise = pMap(Array.from(packageConfigs).sort(), globPath => {
      let chain = globby(path.posix.join(globPath, fileName), options); // fast-glob does not respect pattern order, so we re-sort by absolute path

      chain = chain.then(results => results.sort()); // POSIX results always need to be normalized
...
function makeSyncFileFinder(rootPath, packageConfigs) {
  ...
  return (fileName, fileMapper, customGlobOpts) => {
    ...
    let results = globby.sync(patterns, options); // POSIX results always need to be normalized
