...
const {
  applyExtends
} = require("./lib/apply-extends");

const {
  deprecateConfig
} = require("./lib/deprecate-config");

const {
  makeFileFinder,
  makeSyncFileFinder
} = require("./lib/make-file-finder");
/**
...
class Project {
  ...
  get manifest() {
    ...
    try {
      ...
      manifest = new Package(packageJson, this.rootPath); // redefine getter to lazy-loaded value
  ...
  get licensePath() {
    ...
    try {
      ...
      if (licensePath) {
        ...
        licensePath = path.normalize(licensePath); // redefine getter to lazy-loaded value
  ...
  get fileFinder() {
    const finder = makeFileFinder(this.rootPath, this.packageConfigs); // redefine getter to lazy-loaded value
  ...
  getPackages() {
    const mapper = packageConfigPath => loadJsonFile(packageConfigPath).then(packageJson => new Package(packageJson, path.dirname(packageConfigPath), this.rootPath));

    return this.fileFinder("package.json", filePaths => pMap(filePaths, mapper, {
      concurrency: 50
    }));
  }
  /**
  ...
  getPackageLicensePaths() {
    return this.fileFinder(Project.LICENSE_GLOB, null, {
      caseSensitiveMatch: false
    });
  }
