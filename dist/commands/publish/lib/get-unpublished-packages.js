...
function getUnpublishedPackages(packageGraph, opts) {
  ...
  let chain = Promise.resolve(); // don't bother attempting to get the packument for private packages

  const graphNodesToCheck = Array.from(packageGraph.values()).filter(({
    pkg
  }) => !pkg.private);

  const mapper = pkg => pacote.packument(pkg.name, opts).then(packument => {
  ...
  chain = chain.then(() => pMap(graphNodesToCheck, mapper, {
    concurrency: 4
  }));
