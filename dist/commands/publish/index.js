...
const {
  describeRef
} = require("@lerna/describe-ref");

const {
  throwIfUncommitted
} = require("@lerna/check-working-tree");

const {
  promptConfirmation
} = require("@lerna/prompt");

const {
  output
} = require("@lerna/output");

const {
  collectUpdates
} = require("@lerna/collect-updates");
...
const {
  npmPublish
} = require("@lerna/npm-publish");

const {
  packDirectory
} = require("@lerna/pack-directory");

const {
  logPacked
} = require("@lerna/log-packed");
...
} = require("@lerna/prerelease-id-from-version");

const {
  getOneTimePassword
} = require("@lerna/otplease");

const {
  createTempLicenses
} = require("./lib/create-temp-licenses");

const {
  getCurrentSHA
} = require("./lib/get-current-sha");

const {
  getCurrentTags
} = require("./lib/get-current-tags");

const {
  getUnpublishedPackages
} = require("./lib/get-unpublished-packages");

const {
  getNpmUsername
} = require("./lib/get-npm-username");

const {
  getTaggedPackages
} = require("./lib/get-tagged-packages");

const {
  getPackagesWithoutLicense
} = require("./lib/get-packages-without-license");

const {
  gitCheckout
} = require("./lib/git-checkout");

const {
  removeTempLicenses
} = require("./lib/remove-temp-licenses");

const {
  verifyNpmPackageAccess
} = require("./lib/verify-npm-package-access");

const {
  getTwoFactorAuthRequired
} = require("./lib/get-two-factor-auth-required");
...
class PublishCommand extends Command {
  ...
  configureProperties() {
    super.configureProperties(); // Defaults are necessary here because yargs defaults
    ...
    this.savePrefix = exact ? "" : "^"; // https://docs.npmjs.com/misc/config#tag-version-prefix

    this.tagPrefix = tagVersionPrefix; // TODO: properly inherit from npm-conf
    // inverted boolean options are only respected if prefixed with `--no-`, e.g. `--no-verify-access`

    this.gitReset = gitReset !== false;
    this.verifyAccess = verifyAccess !== false; // consumed by npm-registry-fetch (via libnpmpublish)
  ...
  initialize() {
    ...
    this.runPackageLifecycle = createRunner(this.options); // don't execute recursively if run from a poorly-named script
    ...
    return chain.then(result => {
      ...
      if (!result.updates.length) {
        this.logger.success("No changed packages to publish"); // still exits zero, aka "ok"
  ...
  detectFromGit() {
    ...
    let chain = Promise.resolve(); // attempting to publish a tagged release with local changes is not allowed
  ...
  detectFromPackage() {
    let chain = Promise.resolve(); // attempting to publish a release with local changes is not allowed
  ...
  detectCanaryVersions() {
    ...
    const {
      ...
      forcePublish,
      includeMergedTags
    } = this.options; // "prerelease" and "prepatch" are identical, for our purposes

    const release = bump.startsWith("pre") ? bump.replace("release", "patch") : `pre${bump}`;
    let chain = Promise.resolve(); // attempting to publish a canary release with local changes is not allowed

    chain = chain.then(() => this.verifyWorkingTreeClean()); // find changed packages since last release, if any
    ...
    chain = chain.then(() => collectUpdates(this.packageGraph.rawPackageList, this.packageGraph, this.execOpts, {
      ...
      ignoreChanges,
      forcePublish,
      includeMergedTags // private packages are never published, don't bother describing their refs.

    }).filter(node => !node.pkg.private));

    const makeVersion = fallback => ({
      lastVersion = fallback,
      refCount,
      sha
    }) => {
      // the next version is bumped without concern for preid or current index
      const nextVersion = semver.inc(lastVersion.replace(this.tagPrefix, ""), release.replace("pre", "")); // semver.inc() starts a new prerelease at .0, git describe starts at .1
    ...
    if (this.project.isIndependent()) {
      ...
      chain = chain.then(updates => pMap(updates, node => describeRef({
        match: `${node.name}@*`,
        cwd
      }, includeMergedTags) // an unpublished package will have no reachable git tag
      .then(makeVersion(node.version)).then(version => [node.name, version])).then(updatesVersions => ({
        updates,
        updatesVersions
      })));
    } else {
      ...
      chain = chain.then(updates => describeRef({
        ...
        cwd
      }, includeMergedTags) // a repo with no tags should default to whatever lerna.json claims
  ...
  prepareLicenseActions() {
    return Promise.resolve().then(() => getPackagesWithoutLicense(this.project, this.packagesToPublish)).then(packagesWithoutLicense => {
      if (packagesWithoutLicense.length && !this.project.licensePath) {
        ...
        const list = names.length > 1 ? `${names.slice(0, -1).join(", ")}${names.length > 2 ? "," : ""} and ${names[names.length - 1]
        /* oxford commas _are_ that important */
        }` : names[0];
  ...
  updateCanaryVersions() {
    return pMap(this.updates, node => {
      ...
      for (const [depName, resolved] of node.localDependencies) {
        ...
        const depVersion = this.updatesVersions.get(depName) || this.packageGraph.get(depName).pkg.version; // it no longer matters if we mutate the shared Package instance
  ...
  resetChanges() {
    ...
    const dirtyManifests = [this.project.manifest].concat(this.packagesToPublish).map(pkg => path.relative(cwd, pkg.manifestLocation));
  ...
  removeTempLicensesOnError(error) {
    return Promise.resolve().then(() => removeTempLicenses(this.packagesToBeLicensed).catch(removeError => {
  ...
  packUpdated() {
    ...
    if (!this.hasRootedLeaf) {
      ...
      chain = chain.then(() => this.runRootLifecycle("prepublish")); // these lifecycles _should_ never be employed to run `lerna publish`...
    ...
    const mapper = pPipe(...[this.options.requireScripts && (pkg => this.execScript(pkg, "prepublish")), pkg => pulseTillDone(packDirectory(pkg, pkg.location, opts)).then(packed => {
      tracker.verbose("packed", path.relative(this.project.rootPath, pkg.contents));
      tracker.completeWork(1); // store metadata for use in this.publishPacked()

      pkg.packed = packed; // manifest may be mutated by any previous lifecycle

      return pkg.refresh();
    })].filter(Boolean));
    chain = chain.then(() => this.topoMapPackages(mapper));
    chain = chain.then(() => removeTempLicenses(this.packagesToBeLicensed)); // remove temporary license files if _any_ error occurs _anywhere_ in the promise chain
  ...
  publishPacked() {
    ...
    tracker.addWork(this.packagesToPublish.length);
    let chain = Promise.resolve(); // if account-level 2FA is enabled, prime the OTP cache
    ...
    const mapper = pPipe(...[pkg => {
      const preDistTag = this.getPreDistTag(pkg);
      const tag = !this.options.tempTag && preDistTag ? preDistTag : opts.tag;
      const pkgOpts = Object.assign({}, opts, {
        tag
      });
    ...
    }, this.options.requireScripts && (pkg => this.execScript(pkg, "postpublish"))].filter(Boolean));
    chain = chain.then(() => this.topoMapPackages(mapper));
