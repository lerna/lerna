...
function hasDependencyInstalled(pkg, depName, needVersion) {
  ...
  return getInstalled(pkg).then(versions => versions.has(depName) && semver.satisfies(versions.get(depName), needVersion));
}
/**
...
function getInstalled(pkg) {
  return new Promise((resolve, reject) => {
    ...
    readPackageTree(pkg.location, filterTopLevel, (err, {
      children
    }) => {
      if (err) {
        return reject(err);
      ...
      const deps = new Map(children.map(({
        package: {
          name,
          version
        }
...
function filterTopLevel(node, kidName) {
  ...
  return Boolean(node.package.dependencies && node.package.dependencies[kidName] || node.package.devDependencies && node.package.devDependencies[kidName] || node.package.peerDependencies && node.package.peerDependencies[kidName] || node.package.optionalDependencies && node.package.optionalDependencies[kidName]);
}