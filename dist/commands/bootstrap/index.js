...
const {
  Command
} = require("@lerna/command");

const {
  rimrafDir
} = require("@lerna/rimraf-dir");

const {
  hasNpmVersion
} = require("@lerna/has-npm-version");

const {
  npmInstall,
  npmInstallDependencies
} = require("@lerna/npm-install");

const {
  createRunner
} = require("@lerna/run-lifecycle");

const {
  runTopologically
} = require("@lerna/run-topologically");

const {
  symlinkBinary
} = require("@lerna/symlink-binary");

const {
  symlinkDependencies
} = require("@lerna/symlink-dependencies");

const {
  ValidationError
} = require("@lerna/validation-error");

const {
  getFilteredPackages
} = require("@lerna/filter-options");

const {
  PackageGraph
} = require("@lerna/package-graph");

const {
  pulseTillDone
} = require("@lerna/pulse-till-done");

const {
  hasDependencyInstalled
} = require("./lib/has-dependency-installed");

const {
  isHoistedPackage
} = require("./lib/is-hoisted-package");
...
class BootstrapCommand extends Command {
  ...
  initialize() {
    ...
    const {
      LERNA_EXEC_PATH = "leaf",
      LERNA_ROOT_PATH = "root"
    } = process.env;
    ...
    this.runPackageLifecycle = createRunner({
      registry
    });
    this.npmConfig = {
      registry,
    ...
    chain = chain.then(filteredPackages => {
      ...
      if (filteredPackages.length !== this.targetGraph.size && !this.options.forceLocal) {
        this.logger.warn("bootstrap", "Installing local packages that do not match filters from registry"); // an explicit --scope, --ignore, or --since should only symlink the targeted packages, no others

        this.targetGraph = new PackageGraph(filteredPackages, "allDependencies", this.options.forceLocal); // never automatically --save or modify lockfiles

        this.npmConfig.npmClientArgs.unshift(npmClient === "yarn" ? "--pure-lockfile" : "--no-save"); // never attempt `npm ci`, it would always fail
  ...
  execute() {
    ...
    const scriptsEnabled = this.options.ignoreScripts !== true; // root install does not need progress bar

    this.enableProgressBar();
    this.logger.info("", `Bootstrapping ${packageCountLabel}`); // conditional task queue
    ...
    tasks.push(() => this.getDependenciesToInstall(), result => this.installExternalDependencies(result), () => this.symlinkPackages());

    if (scriptsEnabled) {
      tasks.push(() => this.runLifecycleInPackages("install"), () => this.runLifecycleInPackages("postinstall"));
  ...
  installRootPackageOnly() {
    this.logger.info("bootstrap", "root only"); // don't hide yarn or npm output
  ...
  rootHasLocalFileDependencies() {
    ...
    return Object.keys(rootDependencies).some(name => this.targetGraph.has(name) && npa.resolve(name, rootDependencies[name], this.project.rootPath).type === "directory");
  }
  ...
  getDependenciesToInstall() {
    ...
    const rootPkg = this.project.manifest; // This will contain entries for each hoistable dependency.

    const rootSet = new Set(); // This will map packages to lists of unhoistable dependencies
    ...
    const filteredNodes = new Map(this.filteredPackages.map(pkg => [pkg.name, this.targetGraph.get(pkg.name)])); // collect root dependency versions

    const mergedRootDeps = Object.assign({}, rootPkg.devDependencies, rootPkg.optionalDependencies, rootPkg.dependencies);
    const rootExternalVersions = new Map(Object.keys(mergedRootDeps).map(externalName => [externalName, mergedRootDeps[externalName]])); // seed the root dependencies
    ...
    for (const [leafName, leafNode] of filteredNodes) {
      for (const [externalName, resolved] of leafNode.externalDependencies) {
        ...
        const record = depsToInstall.get(externalName) || depsToInstall.set(externalName, new Map()).get(externalName);
    ...
    const rootActions = [];
    const leafActions = []; // We don't want to exit on the first hoist issue, but log them all and then exit

    let strictExitOnWarning = false; // determine where each dependency will be installed

    for (const [externalName, externalDependents] of depsToInstall) {
      let rootVersion;

      if (this.hoisting && isHoistedPackage(externalName, this.hoisting)) {
        const commonVersion = Array.from(externalDependents.keys()).reduce((a, b) => externalDependents.get(a).size > externalDependents.get(b).size ? a : b); // Get the version required by the repo root (if any).
        ...
        if (rootVersion !== commonVersion) {
          this.logger.warn("EHOIST_ROOT_VERSION", `The repository root depends on ${externalName}@${rootVersion}, ` + `which differs from the more common ${externalName}@${commonVersion}.`);
        ...
        const dependents = Array.from(externalDependents.get(rootVersion)).map(leafName => this.targetGraph.get(leafName).pkg); // remove collection so leaves don't repeat it

        externalDependents.delete(rootVersion); // Install the best version we can in the repo root.
        ...
        rootActions.push(() => hasDependencyInstalled(rootPkg, externalName, rootVersion).then(isSatisfied => {
          rootSet.add({
            name: externalName,
            dependents,
      ...
      for (const [leafVersion, leafDependents] of externalDependents) {
        for (const leafName of leafDependents) {
          if (rootVersion) {
            this.logger.warn("EHOIST_PKG_VERSION", `"${leafName}" package depends on ${externalName}@${leafVersion}, ` + `which differs from the hoisted ${externalName}@${rootVersion}.`);
          ...
          const leafRecord = leaves.get(leafNode) || leaves.set(leafNode, new Set()).get(leafNode); // only install dependency if it's not already installed

          leafActions.push(() => hasDependencyInstalled(leafNode.pkg, externalName, leafVersion).then(isSatisfied => {
            leafRecord.add({
    ...
    if (this.options.strict && strictExitOnWarning) {
      throw new ValidationError("EHOISTSTRICT", "Package version inconsistencies found while hoisting. Fix the above warnings and retry.");
    }
  ...
  installExternalDependencies({
    ...
    const actions = []; // Start root install first, if any, since it's likely to take the longest.
    ...
    if (rootSet.size) {
      ...
      actions.push(() => {
        const depsToInstallInRoot = root.some(({
          isSatisfied
        }) => !isSatisfied) ? root.map(({
          dependency
        }) => dependency) : [];
        ...
        return pulseTillDone(promise).then(() => // Link binaries into dependent packages so npm scripts will
        // have access to them.
        pMapSeries(root, ({
          name,
          dependents
      ...
      actions.push(() => {
        ...
        const candidates = root.filter(dep => dep.dependents.length).reduce((list, {
          name,
          dependents
        }) => {
          const dirs = dependents.filter(pkg => pkg.nodeModulesLocation !== rootPkg.nodeModulesLocation).map(pkg => path.join(pkg.nodeModulesLocation, name));
        ...
        return pMap(candidates, dirPath => pulseTillDone(rimrafDir(dirPath)).then(() => {
    ...
    leaves.forEach((leafRecord, leafNode) => {
      const deps = Array.from(leafRecord); // If we have any unsatisfied deps then we need to install everything.
  ...
  symlinkPackages() {
    return symlinkDependencies(this.filteredPackages, this.targetGraph, this.logger.newItem("bootstrap dependencies"));
  }
