...
class ImportCommand extends Command {
  ...
  initialize() {
    ...
    const packageJson = path.join(externalRepoPath, "package.json"); // eslint-disable-next-line import/no-dynamic-require, global-require
    ...
    if (this.getPackageDirectories().indexOf(targetBase) === -1) {
      throw new ValidationError("EDESTDIR", `--dest does not match with the package directories: ${this.getPackageDirectories()}`);
    }

    const targetDir = path.join(targetBase, externalRepoBase); // Compute a target directory relative to the Git root
  ...
  createPatchForCommit(sha) {
    ...
    if (this.options.flatten) {
      const diff = this.externalExecSync("git", ["log", "--reverse", "--first-parent", "-p", "-m", "--pretty=email", "--stat", "--binary", "-1", "--color=never", sha, // custom git prefixes for accurate parsing of filepaths (#1655)
      `--src-prefix=COMPARE_A/`, `--dst-prefix=COMPARE_B/`]);
      const version = this.externalExecSync("git", ["--version"]).replace(/git version /g, "");
      patch = `${diff}\n--\n${version}`;
    } else {
      patch = this.externalExecSync("git", ["format-patch", "-1", sha, "--stdout", // custom git prefixes for accurate parsing of filepaths (#1655)
      `--src-prefix=COMPARE_A/`, `--dst-prefix=COMPARE_B/`]);
    }

    const formattedTarget = this.targetDirRelativeToGitRoot.replace(/\\/g, "/");
    const replacement = `$1/${formattedTarget}`; // Create a patch file for this commit and prepend the target directory
  ...
  execute() {
    ...
    const mapper = sha => {
      ...
      return pulseTillDone(proc).then(() => {
        ...
        if (diff === "") {
          tracker.completeWork(1); // Automatically skip empty commits
    ...
    return pMapSeries(this.commits, mapper).then(() => {
      tracker.finish();
      ...
      this.logger.error("import", `Rolling back to previous HEAD (commit ${this.preImportHead})`); // Abort the failed `git am` and roll back to previous HEAD.

      this.execSync("git", ["am", "--abort"]);
      this.execSync("git", ["reset", "--hard", this.preImportHead]);
      throw new ValidationError("EIMPORT", dedent`
