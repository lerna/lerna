...
const {
  getRangeToReference
} = require("./lib/get-range-to-reference");
...
class AddCommand extends Command {
  ...
  initialize() {
    ...
    this.selfSatisfied = this.packageSatisfied(); // https://docs.npmjs.com/misc/config#save-prefix
  ...
  collectPackagesToChange() {
    ...
    let result = this.filteredPackages; // Skip packages that only would install themselves
    ...
    result = result.filter(pkg => {
      const deps = this.getPackageDeps(pkg); // Check if one of the packages to install necessitates a change to pkg's manifest
  ...
  packageSatisfied() {
    ...
    this.spec.saveRelativeFileSpec = Array.from(this.packageGraph.values()).some(node => node.localDependencies.size && Array.from(node.localDependencies.values()).some(resolved => resolved.type === "directory"));
