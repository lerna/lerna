...
const {
  recommendVersion,
  updateChangelog
} = require("@lerna/conventional-commits");
...
const {
  getCurrentBranch
} = require("./lib/get-current-branch");
...
const {
  gitCommit
} = require("./lib/git-commit");

const {
  gitPush
} = require("./lib/git-push");
...
const {
  isBehindUpstream
} = require("./lib/is-behind-upstream");

const {
  remoteBranchExists
} = require("./lib/remote-branch-exists");

const {
  isBreakingChange
} = require("./lib/is-breaking-change");

const {
  isAnythingCommitted
} = require("./lib/is-anything-committed");

const {
  makePromptVersion
} = require("./lib/prompt-version");

const {
  createRelease,
  createReleaseClient
} = require("./lib/create-release");
...
class VersionCommand extends Command {
  ...
  configureProperties() {
    ...
    this.pushToRemote = gitTagVersion && amend !== true && push; // never automatically push to remote when amending a commit
  ...
  initialize() {
    ...
    if (this.requiresGit) {
      ...
      if (this.options.allowBranch && ![].concat(this.options.allowBranch).some(x => minimatch(this.currentBranch, x))) {
      ...
      if (this.commitAndTag && this.pushToRemote && isBehindUpstream(this.gitRemote, this.currentBranch, this.execOpts)) {
        ...
        this.logger.warn("EBEHIND", `${message}, exiting`); // still exits zero, aka "ok"
    ...
    this.updates = collectUpdates(this.packageGraph.rawPackageList, this.packageGraph, this.execOpts, this.options).filter(node => {
    ...
    if (!this.updates.length) {
      this.logger.success(`No changed packages to ${this.composed ? "publish" : "version"}`); // still exits zero, aka "ok"
    ...
    const tasks = [() => this.getVersionsForUpdates(), versions => this.setUpdatesForVersions(versions), () => this.confirmVersions()]; // amending a commit probably means the working tree is dirty
  ...
  execute() {
    ...
    if (this.releaseClient) {
      ...
      tasks.push(() => createRelease(this.releaseClient, {
        tags: this.tags,
  ...
  getPrereleasePackageNames() {
    ...
    return collectPackages(this.packageGraph, {
      isCandidate
    }).map(pkg => pkg.name);
  }
  ...
  recommendVersions(resolvePrereleaseId) {
    ...
    chain = chain.then(() => this.reduceVersions(node => recommendVersion(node, type, {
      changelogPreset,
      rootPath,
  ...
  setUpdatesForVersions(versions) {
    if (this.project.isIndependent() || versions.size === this.packageGraph.size) {
      ...
      if (hasBreakingChange) {
        ...
        this.updates = Array.from(this.packageGraph.values()); // --no-private completely removes private packages from consideration
  ...
  updatePackageVersions() {
    ...
    const independentVersions = this.project.isIndependent();
    const rootPath = this.project.manifest.location;
    const changedFiles = new Set(); // my kingdom for async await :(

    let chain = Promise.resolve(); // preversion:  Run BEFORE bumping the package version.
    ...
    const actions = [pkg => this.runPackageLifecycle(pkg, "preversion").then(() => pkg), // manifest may be mutated by any previous lifecycle
    pkg => pkg.refresh(), pkg => {
      // set new version
      pkg.set("version", this.updatesVersions.get(pkg.name)); // update pkg dependencies
    ...
    if (conventionalCommits && changelog) {
      ...
      actions.push(pkg => updateChangelog(pkg, type, {
        ...
        changedFiles.add(logPath); // add release notes
  ...
  commitAndTagUpdates() {
    ...
    chain = chain.then(() => pMap(this.packagesToVersion, pkg => this.runPackageLifecycle(pkg, "postversion")));
  ...
  gitCommitAndTagVersionForUpdates() {
    ...
    return Promise.resolve().then(() => gitCommit(message, this.gitOpts, this.execOpts)).then(() => Promise.all(tags.map(tag => gitTag(tag, this.gitOpts, this.execOpts)))).then(() => tags);
  }
  ...
  gitCommitAndTagVersion() {
    ...
    return Promise.resolve().then(() => gitCommit(message, this.gitOpts, this.execOpts)).then(() => gitTag(tag, this.gitOpts, this.execOpts)).then(() => [tag]);
  }
