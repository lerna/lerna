...
exports.builder = (yargs, composed) => {
  const opts = {
    ...
    "conventional-graduate": {
      describe: "Version currently prereleased packages to a non-prerelease version." // type must remain ambiguous because it is overloaded (boolean _or_ string _or_ array)

    },
    "conventional-prerelease": {
      describe: "Version changed packages as prereleases when using --conventional-commits." // type must remain ambiguous because it is overloaded (boolean _or_ string _or_ array)
    ...
    "force-publish": {
      describe: "Always include targeted packages in versioning operations, skipping default logic." // type must remain ambiguous because it is overloaded (boolean _or_ string _or_ array)
    ...
    "ignore-changes": {
      describe: ["Ignore changes in files matched by glob(s) when detecting changed packages.", "Pass --no-ignore-changes to completely disable."].join("\n"),
      type: "array"
    },
    "ignore-scripts": {
  ...
  yargs.options(opts); // workaround yargs bug that re-interprets unknown arguments in argv._
