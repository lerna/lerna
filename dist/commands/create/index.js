...
const {
  builtinNpmrc
} = require("./lib/builtin-npmrc");

const {
  catFile
} = require("./lib/cat-file");

const LERNA_MODULE_DATA = require.resolve("./lib/lerna-module-data.js");

const DEFAULT_DESCRIPTION = ["Now I’m the model of a modern major general", "The venerated Virginian veteran whose men are all", "Lining up, to put me up on a pedestal", "Writin’ letters to relatives", "Embellishin’ my elegance and eloquence", "But the elephant is in the room", "The truth is in ya face when ya hear the British cannons go", "BOOM"].join(" / ");
...
class CreateCommand extends Command {
  initialize() {
    const {
      ...
      name: pkgName,
      yes
    } = this.options; // npm-package-arg handles all the edge-cases with scopes

    const {
      name,
      scope
    } = npa(pkgName); // optional scope is _not_ included in the directory name

    this.dirName = scope ? name.split("/").pop() : name;
    this.pkgName = name;
    this.pkgsDir = this.project.packageParentDirs.find(pd => pd.indexOf(pkgLocation) > -1) || this.project.packageParentDirs[0];
    this.camelName = camelCase(this.dirName); // when transpiling, src => dist; otherwise everything in lib
    ...
    this.conf.addFile(builtinNpmrc(), "builtin"); // always set init-main, it's half of the whole point of this module
  ...
  execute() {
    ...
    return chain.then(data => {
      ...
      this.logger.success("create", `New package ${data.name} created at ./${path.relative(".", this.targetDir)}`);
    });
  }
  ...
  setDependencies() {
    const inputs = new Set((this.options.dependencies || []).sort()); // add yargs if a bin is required
    ...
    let chain = Promise.resolve();
    chain = chain.then(() => pReduce(inputs, (obj, input) => {
  ...
  setHomepage() {
    ...
    let {
      homepage = this.project.manifest.get("homepage")
    } = this.options;
    ...
    if (hurl.hostname.match("github")) {
      hurl.pathname = path.posix.join(hurl.pathname, "tree/main", relativeTarget); // TODO: get actual upstream HEAD branch name
  ...
  writeCliFile() {
    ...
    const cliContent = [this.options.esModule ? dedent`
  ...
  writeCliTest() {
    ...
    const cliTestContent = [this.options.esModule ? dedent`
  ...
  writeBinFile() {
    ...
    return catFile(this.binDir, this.binFileName, binContent, {
      mode: 0o755
    });
  }
