...
class RunCommand extends Command {
  ...
  initialize() {
    ...
    chain = chain.then(filteredPackages => {
      this.packagesWithScript = script === "env" ? filteredPackages : filteredPackages.filter(pkg => pkg.scripts && pkg.scripts[script]);
    });
    return chain.then(() => {
      ...
      if (!this.count) {
        this.logger.success("run", `No packages found with the lifecycle script '${script}'`); // still exits zero, aka "ok"
  ...
  execute() {
    ...
    if (this.bail) {
      ...
      chain = chain.catch(err => {
        process.exitCode = err.exitCode; // rethrow to halt chain and log properly
    ...
    return chain.then(() => {
      this.logger.success("run", "Ran npm script '%s' in %d %s in %ss:", this.script, this.count, this.packagePlural, (getElapsed() / 1000).toFixed(1));
