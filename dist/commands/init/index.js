...
class InitCommand extends Command {
  ...
  ensurePackageJSON() {
    ...
    if (!this.project.manifest) {
      this.logger.info("", "Creating package.json"); // initialize with default indentation so write-pkg doesn't screw it up with tabs

      chain = chain.then(() => writeJsonFile(path.join(this.project.rootPath, "package.json"), {
        name: "root",
        private: true
      }, {
        indent: 2
