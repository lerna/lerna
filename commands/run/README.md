...
```sh
$ lerna run --force-local build
```

When passed, this flag causes the `run` command to always run the script in topological order regardless of matching version range.
