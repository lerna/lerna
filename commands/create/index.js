...
class CreateCommand extends Command {
  ...
  execute() {
    ...
    chain = chain.then((data) => this.injectRepositoryDirectory(data));
  ...
  injectRepositoryDirectory(data) {
    // package.json is generated by init-package-json, which does not support adding a repository.directory field, so we have to add the field to the file ourselves.

    if (!data.repository) {
      return data;
    }

    const dataWithDirectory = {
      ...data,
      repository: {
        ...data.repository,
        directory: path
          .relative(this.project.rootPath, this.targetDir)
          // package.json uses uniq/posix slashes, not windows
          .replace("\\", "/"),
      },
    };

    return catFile(this.targetDir, "package.json", JSON.stringify(dataWithDirectory, null, 2)).then(
      () => dataWithDirectory
    );
  }
