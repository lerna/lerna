...
function userGetVersion(currentVersion, pkgName, repoVersion) {
  var message = "New version ";
  if (pkgName) message += "for \"" + pkgName + "\" ";
  message += "(Leave blank for patch version): ";

  var input = repoVersion || readline.question(message);
...
module.exports = function publishChangedPackages(
  ...
  try {
    ...
    if (flags.canary) {
      ...
      masterVersion = userGetVersion(currentVersion, undefined, flags.repoVersion);
