...
var getCurrentSHA            = require("../../utils/getCurrentSHA");
  ...
  try {
    ...
    changedPackages.forEach(function(pkg) {
      if (flags.independent) {
        ...
        versions[pkg.name] = masterVersion + "-canary." + getCurrentSHA().slice(0, 8);
      } else {
    ...
    updateChangedPackages(changedPackages, packagesLoc, versions, flags);
