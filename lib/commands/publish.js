...
function gitAddVersionTag(version) {
...
exports.execute = function (config) {
  ...
  var version = userGetVersion(config.currentVersion);

  writeFile(config.versionLoc, version, "utf8");
  ...
  try {
    ...
    updateChangedPackages(changedPackages, config.packagesLoc, version);

    // Add a tag for the new version
    gitAddVersionTag(version);
    createdTag = true;
    ...
    npmPublishAsPrerelease(changedPackages, config.packagesLoc, function (err) {
      ...
      npmSetLatest(changedPackages, version, function (err) {
        if (err) return onError(err);
        execSync("git push");
